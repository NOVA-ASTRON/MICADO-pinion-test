...
typedef struct optional_devs_t {
    const deviceinfo * infos;
    int num_infos;
} optional_devs;
...
public:
       ...
       QSerialPort *FindSerialPort(const optional_devs *devs);

       bool Connect();

       void Disconnect();
       void InitDriverDev(const optional_devs *ID, LogFile l, std::string n);
